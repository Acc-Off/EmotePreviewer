...
dotnet run -c Release --no-build --project tools/EmotePreviewer.DevTools -- ycd <dict> [out.ycd]      # クリップ辞書をアーカイブから取り出す
dotnet run -c Release --no-build --project tools/EmotePreviewer.DevTools -- clipinfo <dict> <clip> [--dump out.json]  # クリップのタイミング・アニメーションヘッダ・チャネル種別。--dump で全フレームの生トラック値
