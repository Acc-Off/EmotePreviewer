...
dotnet run -c Release --no-build --project tools/EmotePreviewer.DevTools -- ycd <dict> [out.ycd]      # extract a clip dictionary from the archives
dotnet run -c Release --no-build --project tools/EmotePreviewer.DevTools -- clipinfo <dict> <clip> [--dump out.json]  # clip timing, animation headers, channel types; --dump = every frame's raw track values
